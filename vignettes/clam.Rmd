...
To use `clam`, first the package has to be loaded (this will also load its companion packages `rice` and `rintcal` which deal with radiocarbon calibration):
...
2+2
3+3
# Create a temporary directory for clam_runs
clam_temp_dir <- file.path(tempdir(), "clam_runs")
if (!dir.exists(clam_temp_dir)) {
  dir.create(clam_temp_dir)
}

# Create a subdirectory within clam_runs
example_dir <- file.path(clam_temp_dir, "Example")
if (!dir.exists(example_dir)) {
  dir.create(example_dir)
}

# Assign the coredir to the temporary example directory
coredir <- example_dir

print(paste("Current working directory:", getwd()))
print(paste("Temporary directory:", tempdir()))
print(paste("Coredir provided:", coredir))

if (!is.null(coredir)) {
  print(paste("Directory exists:", dir.exists(coredir)))
  print(paste("Writable:", file.access(coredir, 2) == 0))
}
```

```{r, echo=FALSE, fig.width=5, fig.height=5}
dir.create("clam_runs")
dir.create("clam_runs/Example")
clam(ask=FALSE, coredir=".")
```
